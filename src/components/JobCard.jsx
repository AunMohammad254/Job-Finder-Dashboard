...
import { Heart, MapPin, Briefcase, DollarSign, Calendar, ArrowRight } from 'lucide-react';
...
const jobTypeColors = {
  ...
  'Remote': 'bg-purple-500/10 text-purple-400 border-purple-500/30',
  'Internship': 'bg-amber-500/10 text-amber-400 border-amber-500/30',
  'Contract': 'bg-cyan-500/10 text-cyan-400 border-cyan-500/30'
};
...
export default function JobCard({ job }) {
  ...
  // Extract initials for company fallback logo
  ...
  return (
    <div className="group relative flex flex-col justify-between bg-zinc-900/70 hover:bg-zinc-900/90 border border-zinc-800/80 hover:border-purple-500/40 rounded-2xl p-5 sm:p-6 transition-all duration-300 hover:shadow-xl hover:shadow-purple-950/20 hover:-translate-y-1 backdrop-blur-sm">
      {/* Top Header */}
      <div>
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-white group-hover:text-purple-300 transition-colors text-base leading-snug line-clamp-1">
                <Link to={`/jobs/${job.id}`}>
                  {job.title}
                </Link>
              </h3>
          ...
          <button
            ...
            className={`p-2.5 rounded-xl border transition-all duration-200 cursor-pointer ${
              saved
                ...
                : 'bg-zinc-800/60 hover:bg-zinc-800 border-zinc-700/60 text-zinc-400 hover:text-white'
            }`}
          >
            <Heart
              className={`w-4 h-4 transition-transform duration-200 active:scale-125 ${
                saved ? 'fill-current text-rose-400' : ''
              }`}
            />
          </button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4 text-xs">
          <span
            className={`px-2.5 py-1 rounded-full font-medium border ${
              jobTypeColors[job.jobType] || 'bg-zinc-800 text-zinc-300 border-zinc-700'
            }`}
          >
            {job.jobType}
          </span>
          <span className="flex items-center gap-1 text-zinc-400 bg-zinc-800/50 px-2.5 py-1 rounded-full border border-zinc-800">
            <MapPin className="w-3 h-3 text-zinc-400" />
            {job.location}
          </span>
          <span className="flex items-center gap-1 text-emerald-400 bg-emerald-950/30 px-2.5 py-1 rounded-full border border-emerald-900/50 font-medium">

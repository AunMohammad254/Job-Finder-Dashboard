  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      <div className="bg-zinc-900/80 border border-zinc-800 rounded-3xl p-6 sm:p-10 backdrop-blur-xl shadow-2xl shadow-purple-950/20 mb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 pb-8 border-b border-zinc-800">
          <div className="flex items-start gap-4 sm:gap-5">
            ...
            <div
              ...
              } p-0.5 shadow-xl flex-shrink-0 flex items-center justify-center`}
            >
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2 mb-2">
                <span className="px-2.5 py-0.5 text-xs font-semibold rounded-full bg-purple-500/15 border border-purple-500/30 text-purple-300">
                  {job.jobType}
                </span>
                {job.featured && (
                  <span className="px-2.5 py-0.5 text-xs font-semibold rounded-full bg-amber-500/15 border border-amber-500/30 text-amber-300 flex items-center gap-1">
          ...
          <div className="flex items-center gap-3 self-start md:self-auto w-full md:w-auto">
            <button
              ...
              onClick={handleShare}
              title="Share job"
              className="p-3 rounded-xl bg-zinc-800/80 hover:bg-zinc-800 border border-zinc-700 text-zinc-400 hover:text-white transition-colors cursor-pointer"
            >
            ...
            <button
              ...
              className={`p-3 rounded-xl border transition-all cursor-pointer ${
                saved
                  ...
                  : 'bg-zinc-800/80 hover:bg-zinc-800 border-zinc-700 text-zinc-400 hover:text-white'
              }`}
            >
              <Heart className={`w-5 h-5 ${saved ? 'fill-current text-rose-400' : ''}`} />
            </button>
        ...
        {/* Highlight Compensation Box */}
        <div className="my-8 p-5 rounded-2xl bg-zinc-950/60 border border-zinc-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {job.benefits.map((benefit, idx) => (
                  <div
                    key={idx}
                    className="p-3.5 rounded-xl bg-zinc-950/40 border border-zinc-800/80 text-sm text-zinc-300 flex items-center gap-2.5"
                  >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {job.skills?.map((skill, idx) => (
                <span
                  key={idx}
                  className="px-3 py-1.5 rounded-xl bg-purple-500/10 border border-purple-500/30 text-purple-300 font-medium text-xs sm:text-sm"
                >
                  {skill}
                </span>
              ))}
      ...
      {showApplyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
          <div className="relative w-full max-w-md bg-zinc-900 border border-zinc-700 rounded-3xl p-6 sm:p-8 shadow-2xl shadow-purple-950/60">
            <button
              type="button"
